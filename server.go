...
type ServerWorker struct {
	VersionErrorHandler func(ctx context.Context, ver byte, conn net.Conn)
...
func ReplyErrorByVersion(ctx context.Context, ver byte, conn net.Conn) {
	defer conn.Close()
	switch ver {
	// socks4
	case 4:
...
func (s *ServerWorker) ServeStream(
	...
	if err != nil {
		...
		if errors.Is(err, message.ErrVersion) {
			deferClose = false
			s.VersionErrorHandler(ctx, req.Version, conn)
			return
		}
